...
def get_data(csv_name : str) -> pd.DataFrame:
    ...
    csv_path = os.path.join(base_dir, "..", "..","..","subject", csv_name)
...
def get_and_push():
    ...
    customers = pd.concat([get_data("customer/" + f + ".csv") for f in name_list])
    customers = customers.drop_duplicates(subset=["event_time", "event_type", "product_id", "price", "user_id", "user_session"])
    item = get_data("item/item.csv")
    customers = pd.concat([customers, item], ignore_index=True, axis=0)
